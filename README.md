...
In a Colab notebook, after `git clone` and `pip install -r requirements.txt`,
set your OpenAI key via Colab secrets (key icon in left sidebar → add
`OPENAI_API_KEY` → toggle notebook access), then run:
...
before invoking `app.py` or `agent.py`.
