...
def main() -> None:
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map="auto",
        dtype=torch.bfloat16,  # was `torch_dtype`; renamed in transformers 4.46+
    )
    ...
    sft_config = SFTConfig(
        ...
        bf16=True,
        max_length=config.MAX_SEQ_LEN,  # was `max_seq_length`; renamed in TRL 0.16+
        logging_steps=25,
        eval_strategy="epoch",
        save_strategy="epoch",
        save_total_limit=2,
    ...
    trainer = SFTTrainer(
        ...
        processing_class=tokenizer,  # was `tokenizer=`; renamed in TRL 0.12+
    )
    trainer.train()
